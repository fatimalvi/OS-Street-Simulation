...
pthread_cond_t condOutgoing; /* Conditional Variable for Outgoing Cars */
...
void *street_thread(void *junk) {
	...
	while (1) {
        ...
        while (cars_since_repair + cars_on_street <= USAGE_LIMIT){
	
           
            pthread_cond_wait(&fix, &mutexlock);
...
incoming_enter() {
	 ...
	 *  YOUR CODE HERE. 
	 */
	//printf("incominggggg %d\n", cars_on_street + cars_since_repair);

	pthread_mutex_lock(&mutexlock);

	while(cars_on_street >= ALLOWED_CARS | outgoing_onstreet >= 1 | flag == 1){
	...
	if (cars_on_street + cars_since_repair == USAGE_LIMIT){
		
		flag = 1;
		pthread_cond_signal(&fix);
		
	}

	while (outgoing_onstreet > 0){
		pthread_cond_wait(&condOutgoing, &mutexlock);
...
outgoing_enter() {
	...
	while(cars_on_street >= ALLOWED_CARS | incoming_onstreet >= 1 | flag == 1){
	...
	while (incoming_onstreet > 0){
		pthread_cond_wait(&condIncoming, &mutexlock);
	}
...
incoming_leave() {
	...
	pthread_mutex_unlock(&mutexlock);
	pthread_cond_signal(&condOutgoing);
	pthread_cond_signal(&condlock);
}
...
outgoing_leave() {
	...
	pthread_mutex_unlock(&mutexlock);
    pthread_cond_signal(&condIncoming);
	pthread_cond_signal(&condlock);
